...
#define DSHOT_RPM_FILTER_LPF_HZ 150.0f
...
{
    dshotMotorCount = getMotorCount();
    useDshotTelemetry = motorConfig()->dev.useDshotTelemetry || featureIsEnabled(FEATURE_ESC_SENSOR);
    ...
    for (unsigned i = 0; i < dshotMotorCount; i++) {
        pt1FilterInit(&motorFreqLpf[i], DSHOT_RPM_FILTER_LPF_HZ, US2S(RPM_FILTER_UPDATE_RATE_US));
    }
#else
    UNUSED(looptimeUs);
#endif
...
#else
        motorFrequencyHz[motor] = erpmToHz * getDshotErpm(motor);
#endif
