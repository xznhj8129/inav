...
`src/generated/` contains the generated C wire library, produced from
`schema/*.yaml` by `generator/gen_c.py`: `msp_msgs.h` (packed wire structs,
message ids, and a `MSP_STATIC_ASSERT` pinning every fixed payload size),
`msp_enums.h` (the enums payloads actually reference), and `msp_consts.h`
(schema-declared sizes). Do not edit these; they are regenerated by
`./build_all.sh generate`.

`src/msp_config.h` is hand-written and belongs to this library: it holds the
client-side buffer limits (`MAX_RC_CHANNELS`, `PWM_RANGE_*`, and the
build-dependent values the schema refuses to guess). Override any of them
